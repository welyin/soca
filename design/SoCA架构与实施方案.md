...
版本：0.2，日期：2026-09-17。状态：**可实施设计基线，尚非已实现产品**。本版增加自动拓扑数量调整和认知游戏验收；开发者先读[详细实施规格](SoCA工程实施规格-伸缩与认知游戏.md)。
...
| 需要几层 | L0–L7保留为8类功能职责；单主体默认3级，超过根扇出上限自动插入协调层；多主体再增加社会边界。两种“层”不同 |
| 每层多少单元 | 标准试验配置：64个叶单元、8个能力簇、1个主体，合计73个逻辑单元；通常仅8–16个处于热态，不全部同时推理 |
| 硬件弹性 | 根据当前资源包络和需求实际调整参与拓扑的叶、簇和协调层数量，同时调整驻留与并发；主体身份不随硬件自动复制或合并 |
...
| 如何检验认知与伸缩 | 同一闭环进入迷宫和扫雷，先部分可见符号赛道，再像素赛道；私有真值隔离并在回合中注入可控资源压力 |
...
| R1b | 可选中间协调层 | 默认0；能力簇超过16时每8簇一个协调器 |
| R2 | 完整主体内核 | 1个，协调8簇 |
| R3 | 可选多主体联合体 | 初始不部署；之后2–4个完整主体加轻量协调器 |

**默认分形深度为3，不是8；单主体规模超过128叶时默认插入R1b成为4级，社会边界再加1级。** 节点总数和深度由[伸缩规格](SoCA工程实施规格-伸缩与认知游戏.md)的资源准入、需求和扇出规则决定，不根据硬件任意加深。原SoCA“协作、竞争、轻量集中”是组织原则，不能直接换算成这些数字。叶单元做具体任务，上层额外处理分解、冲突、关系和资源仲裁。

每个能力簇目标8叶，中间协调器最多8簇，主体根最多16个直属孩子。不是先填满一棵树；模板必须支持状态分片/合并，才允许真正改变参与路由的单元数量。实时感知链路不强制穿过所有层，仍必须经过权限代理和审计边界。禁止任务递归无限生成子任务，默认调用深度4、每任务最多32次单元激活，之后请求预算升级或返回部分结果。
...
旧版按总RAM列固定容量，不能完整表达运行中自动调整。本版改为[资源包络→目标拓扑](SoCA工程实施规格-伸缩与认知游戏.md)控制：根据当前可分配RAM/提交量、CPU、选定模型峰值、GPU及任务需求，输出叶数、簇数、中间层数、热集和工作并发。

| Profile | R0叶 | R1簇 | R1b协调 | R2主体 | Total | Depth |
|---|---:|---:|---:|---:|---:|---:|
| T0 | 8 | 1 | 0 | 1 | 10 | 3 |
| T1 | 16 | 2 | 0 | 1 | 19 | 3 |
| T2 | 32 | 4 | 0 | 1 | 37 | 3 |
...
这些是参与当前拓扑的容量，不是永久目录总数，也不等于热态数量。例如从T3降到T1时，64叶/8簇变16叶/2簇，其余单元checkpoint并退出路由，兼容职责迁移到剩余单元；知识与身份不丢弃。硬件允许但任务不需要时不扩容，GPU不足不自动发云。

启动生产最小拓扑为8叶＋1簇＋1主体；原8叶直连主体的9单元演示仅留作P0骨架。标准73个逻辑单元仍不意味着73个模型或并发进程。多主体须先做全局资源分账，不能给每个主体重复分配整机预算，也不能为硬件扩容自行分裂用户主体身份。
...
实现`HardwareSampler → ResourceGovernor → DemandEstimator/TopologyPlanner → TopologyReconciler`。规划器是纯函数，迁移是有版本事务，二者不得混为“改变一个max_workers参数”。[可执行参考](soca_reference/topology.py)已给各层数量计算和伸缩时间门控，[测试](soca_reference/test_topology.py)覆盖13项规划行为；Windows采样和实际迁移尚待实现。

扩容目标稳定60秒、提交冷却120秒，每次最多升一档；缩容稳定10秒可跨档；真实压力/OOM立即冻结新工作，不等待滞后。迁移经过预留、排空、快照、影子恢复、routing epoch提交和旧节点退休，fencing令牌阻止双写。未决外部动作由Broker完成核对，不因迁移重复执行。

具体成本、五组硬件输出、SQL、幂等边界与故障恢复见[详细实施规格第3–8节](SoCA工程实施规格-伸缩与认知游戏.md)。参考代码不创建actor、模型或后台服务，测试通过只证明规划逻辑，不是整个伸缩系统已经运行。
...
增加隔离的`GameHost`：迷宫优先封装MiniGrid，扫雷优先封装成熟Mines内核。SoCA通过同一L0事件、L1记忆、L3候选和Broker动作循环进入游戏，游戏操作只有`game.step`能力，没有任意OS权限。

先用局部可见符号观察，再单独增加像素观察；迷宫不泄露全图/绝对真值，扫雷不泄露雷阵、seed、安全格掩码。Gymnasium的原始info可能含答案，不能直接转发。Evaluator私有控制面负责reset、种子与赛后指标，模型只收到公开观测和可重试的幂等动作接口。

迷宫包括局部探索、钥匙门、延迟线索；扫雷分可推导与风险赛道，正式评测禁Undo/Solve，不因不可避免的猜测失败判定推理必错。训练台提供人工玩/SoCA接管、暂停/单步、公开观测、拓扑计数和资源曲线，调试真值不得进入agent截图或记忆。

将64→16→64叶的资源变化插入同一游戏回合，验证状态迁移、恢复、观察游标和动作去重。对比固定小拓扑、固定大拓扑、自适应拓扑的同预算成绩；分别统计成功、决策校准、时间、token、RAM和迁移开销，不把游戏胜率当AGI证明。

完整规则、Schema、引擎选择、协议、数据隔离、评测矩阵与14张开发工单见[详细实施规格](SoCA工程实施规格-伸缩与认知游戏.md)。本次没有启动或实现游戏应用。
...
|---|---|---|---|
| P0 合同与测试环境 | Schema、事件/动作状态机、纯拓扑规划、迷宫/扫雷引擎锁定、模拟OS | 冻结公开观察与私有真值边界，参考规划和黄金轨迹可复算 | 1–2周 |
| P1 单主体游戏与文字闭环 | 8叶＋1簇＋1主体、迷宫适配、聊天、共享模型网关、只读目录 | 同一闭环能进入游戏和只读任务；扫雷规则适配按工单接入 | 2–3周 |
| P2 可恢复与拓扑伸缩 | SQLite/outbox、快照、职责拆并、epoch迁移、资源控制 | 游戏回合内真实64→16→64叶恢复，故障不重复动作，冷恢复通过 | 3–4周 |
...
P0–P4约13–19周保留为原粗估计；增加引擎适配、像素赛道和拓扑迁移故障覆盖后，应在P0重新估算，不能承诺新增范围没有成本。这不是AGI工期。L7和权重训练在前面的运行时正确性与评价基线完成后再做。

第一批具体任务按顺序为：合同schema和validator；锁定规则引擎；纯拓扑规划；带outbox的事件/动作存储；确定性迷宫闭环；单元生命周期；ModelGateway与聊天；真实迁移和资源控制；扫雷与像素评测；最后扩展设备和桌面写入。具体依赖见[开发工单](SoCA工程实施规格-伸缩与认知游戏.md)。测试和权限不放到“功能做完以后”。
...
| 拓扑数量 | 同需求异资源输出不同叶/簇/协调数；迁移有epoch、fencing、状态恢复和回滚，主体身份不因压力被合并 |
| 认知游戏 | 迷宫局部视野、扫雷公开board、无seed/真值泄露、终局/截断区分、幂等step；结构化与像素成绩分开 |
| 游戏内伸缩 | 固定小/大/自适应三拓扑同预算对照，回合内缩容不丢任务态、不重复动作；故障局不从失败分母删除 |
...
| 默认组合 | 标准64叶/8簇/1主体；按资源和需求在8–512叶间规划，超扇出插入协调层 | 可迁移模板、资源峰值与同预算游戏实验支持改变时 |
