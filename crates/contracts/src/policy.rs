...
impl Approval {
    ...
    pub fn new(
        ...
        // 通道在这里**不判**。§14 那条"不以可能误识别的语音自动批准"是**按等级**的：
        // A3 不收语音批准，而 A2 收（A2 除了审批还有预览、目标版本核对、备份/撤销这几条
        // 确定性兜底，A3 没有）。判定落在 `Approval::covers` 里，因为"这份批准能不能覆盖
        // **这一次**动作"要看动作等级，而构造的时候还不知道它会用在哪一次上。
        //
        // 这一句留在这里是因为我先在这里写了一版一刀切的拒绝（"批准只能来自图形界面或打字"），
        // 而它把 A2 也一并收紧了——`a2_still_accepts_a_voice_approval` 那条测试正是为了防
        // 这个而存在的。写宽和写窄都是错。
        Ok(Self {
            approval_id,
            subject_id,
