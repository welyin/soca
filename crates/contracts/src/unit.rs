...
pub trait CognitiveUnit: Send {
    ...
    /// 向下转型（只读）。见 [`CognitiveUnit::as_any_mut`] 的说明。
    fn as_any(&self) -> &dyn std::any::Any;

    /// 向下转型（可变）。
    ///
    /// **它的用途被限定在装配，不在闭环里。** 装配指的是"把一份配置交给正确的那个叶子"——
    /// 例如能力簇要把一个待推进的动作投递给"待推进动作"那一个槽位，而簇持有的是一列
    /// `Box<dyn CognitiveUnit>`。这是**建好之后就不再发生**的事，一次装配一次调用。
    ///
    /// 明确不许的用法：在 `observe` / `propose` / `predict` / `handle_result` 里的任何一处
    /// 问"你是哪种单元"然后分支。那是把 §4.2 那句"复合单元对外暴露同一份合同"从里面拆掉——
    /// 一旦有分支，簇就不是"一组同构单元"，而是"一个知道每个孩子是什么的调度器"，
    /// 而后者正是 §4.1 明确否掉的 8 级递归树。
    ///
    /// 这条约束靠评审维护，没法用类型表达。之所以仍然加这两个方法而不是让簇按固定下标去摸
    /// 第几个叶单元：把列表顺序变成公开接口更糟——那样加一个单元就会悄悄改变谁被投递到。
